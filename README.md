...
- 🔸 **5a — Evaluation harness first:** a portfolio simulator with walk-forward (out-of-sample) testing, transaction costs + slippage, proper error metrics, and baselines to beat (buy-and-hold and a random walk). No strategy is trusted until it beats those out-of-sample. **Multi-horizon by design:** every signal is evaluated separately at 1-week, 1-month, 3-month, and 1-year horizons, matching the project's weekly-to-long-term scope.
  ...
  - 📊 **First honest result:** on AAPL and SPY (~19 years each), the dashboard's MA rule *underperforms buy-and-hold* on total return (AAPL: 13.3% vs 25.1% CAGR; SPY: 2.1% vs 8.6%) — its one virtue is a much shallower max drawdown (about -35% vs -60%). On SPY it beat only 31% of random-switching strategies. The signal as displayed is a risk-reduction heuristic, not a return-generating one — and now the dashboard's docs say so with numbers.
  - 🔲 Remaining in 5a: multi-ticker sweeps, dividend-adjusted option, signal-transition (event-based) analysis, walk-forward split machinery for when parameters get fitted, and surfacing results in the UI.
