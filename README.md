...
- 🔸 **5a — Evaluation harness first:** a portfolio simulator with walk-forward (out-of-sample) testing, transaction costs + slippage, proper error metrics, and baselines to beat (buy-and-hold and a random walk). No strategy is trusted until it beats those out-of-sample. **Multi-horizon by design:** every signal is evaluated separately at 1-week, 1-month, 3-month, and 1-year horizons, matching the project's weekly-to-long-term scope.
  ...
  - ✅ Harness results surfaced in the UI: each measured ticker's detail panel shows a "signal report card" — following-the-signal CAGR vs buy-and-hold, worst drawdown for both, and the timing-vs-random percentile — generated from the harness (`export.js`) and embedded in the single-file frontend. Unmeasured tickers say so honestly. **Phase 5a is complete.**
- 🔸 **5b — Probabilistic signals:** convert BUY/HOLD/SELL into *calibrated probabilities*; add volatility modeling (GARCH/EWMA — volatility is far more predictable than price); grade with proper scoring rules (Brier score, log-loss).
  - ✅ **Volatility layer shipped — the project's first measured-positive forecast.** EWMA (RiskMetrics λ=0.94, nothing fitted) forecasts next-month volatility with 0.59 rank correlation on our basket, beating both naive baselines 18/18 (`server/harness/vol.js`; digest with citations in `docs/research/volatility.md`). The detail panel now shows "expected swing ±X% · typical month" — bumpiness only; direction stayed unpredictable in every test.
  - ✅ **Up/down probabilities measured — the base rate won.** The planned "base rates + vol layer" probability was tested before shipping (`server/harness/prob.js`, Brier/log-loss vs climatology, 4 horizons × 18 tickers): the textbook drift-over-volatility model (Christoffersen & Diebold 2006) scored *worse* than the plain historical base rate at every horizon (1/18, 0/18, 2/18, 8/18 at 1w/1m/3m/1y; same on total-return prices) — high-vol moments are disproportionately rebound moments, so shrinking P(up) toward 50% in storms fires backwards. Per the pre-committed rule the vol layer stays out of the probabilities; what ships is the measured winner: each basket ticker's report card shows its historical **odds of being higher** at 1w/1m/3m/1y, labeled as base rates, with the receipt that nothing we tested improved on them. Digest: `docs/research/calibration.md`. **Phase 5b is complete** — a fitted probability model (e.g. logit via `walkforward.js`) remains a documented "would change our mind" follow-up, not a promise.
- ✅ **5c — Weighted multi-signal model: measured, and honestly declined.** The planned combination (MA / RSI / MACD / EWMA vol, ridge-regularized logistic regression, every weight fitted only inside `walkforward.js` training windows) was tested before anything shipped (`server/harness/combo.js`, Brier/log-loss vs climatology, 3 horizons × 18 tickers × 15 folds): **0/18 tickers beat the plain base rate at any horizon**, on either price set — and the failure has the textbook shape (damage grows with horizon, shrinks as regularization tightens, fold-to-fold weights can't agree on signs). The literature predicted exactly this for zero-information components (digest: `docs/research/multi-signal.md`), and per the pre-committed ship rule **no weighted score appears in the UI** — the legend records the null instead. Congressional flow was deliberately excluded (known-null aggregate + ~1y of data); per-member records are 5e's job. **Phase 5c is complete.**
- ✅ **5d — Risk & sizing: the project's second measured-positive result.** Sortino and annualized-vol reporting joined the harness (`metrics.js`), and Kelly's lesson was applied the only honest way available: with zero measured directional edge, the Kelly numerator is just base-rate drift — but the *denominator* (variance) is the one thing we can forecast. That's **volatility targeting**: hold `min(1, normal vol / forecast vol)` of the stock, never predicting direction. Measured (`server/harness/voltarget.js`, pre-committed ship rule): beats a matched constant-exposure control on Sharpe **12/18 plain, 16/18 total-return**, cuts the worst drawdown vs buy-and-hold **18/18 on both price sets** (median −37% vs −59%), lost less in 18/17/15 of 18 tickers across the three covered crises, robust to every ablation — at the honest price of ~2.4pp/yr of raw return from ~85% average exposure. Shipped as the report card's "sizing by calm vs. storm" line (measured numbers + live implied exposure, labeled risk illustration, not advice). Digest: `docs/research/risk-sizing.md`. **Phase 5d is complete**; the Phase 6 forward test is the final arbiter (Cederburg et al. 2020's fragility warning is on record).
...
### 🔸 Phase 6 — Paper trading / simulated portfolio ("simulated run") — running, now grading itself
...
- ✅ **The grading report (v0.19.0):** the forward test scoring itself. A panel that takes every trade snapshot and the auto-follow account's rebalance log and grades them against what prices actually did next, at 1w/1m/3m/6m — the volatility band (does the realized month land inside ±1σ about 2 times in 3?), the signal label (**expected to show nothing**, since the harness measured it as carrying no timing information — a confirmed null here is the honest outcome, not a failure), and the vol-sizing account (does it buy lower volatility and a shallower drop with the return it gives up, or neither?). Each grade is the falsifier its research digest pre-registered before shipping.
  - **The grading rules were pre-committed on 2026-07-25, while the accounts were 7 days old and not one horizon had matured** — including the minimum sample sizes below which the panel shows the numbers but refuses to call a verdict (20 matured snapshots for band coverage, 30 per signal bucket, ~6 months of trading days for the sizing account's risk stats). Rules fixed before any result is visible cannot be tuned to flatter it; that is the whole safeguard, and it is written down in `docs/research/README.md` with the date.
  - Returns are measured close-to-close from a single price source rather than from the recorded execution price — the free-tier quote feed runs ~2% off the official close, which at a 1-week horizon would be a larger error than the move being measured. The graded return therefore isn't the account's exact P&L, on purpose.
