...
- **Next planned tasks:** run the digest's `--strategy=macdcross` test (long when MACD line > signal line, else cash — needs the MACD series added to `strategies.js`, kept in sync with server.js) plus the RSI 30-recross event test; then walk-forward machinery and 5b–5e.
