...
## Current state (Phase 5 in progress — 5a and 5b complete, 5c next)
...
- **Next planned tasks:** 5c (weighted multi-signal scoring — note every tested candidate signal so far carries zero timing information, so expect 5c to be mostly a null result honestly displayed), 5d (risk/sizing), 5e (per-member congressional track records with the guardrails from `congressional-trading.md`), then Phase 6 paper trading on the portfolio.js engine.
