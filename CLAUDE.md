...
## Current state (Phases 5 and 6 feature-complete; grading report + Phase 7 next)
...
- The harness results are surfaced in the UI (v0.10.2): the detail panel's "Signal report card" section shows per-ticker measured numbers (signal CAGR vs B&H, drawdowns, matched percentile) from an embedded constant generated by `server/harness/export.js` (regenerate + paste after harness/data changes); non-basket tickers show "not measured yet". **Phase 5a is complete.**
- **Phase 5b opened with the project's first measured-POSITIVE result (v0.11.0):** EWMA volatility (RiskMetrics λ=0.94, nothing fitted) forecasts next-21-day realized vol at 0.59 median rank correlation, beating climatology and persistence baselines 18/18 (harness test `vol.js`, digest `docs/research/volatility.md`). Shipped end-to-end: `ewmaVol()` in server.js (keep in sync with `ewmaVolSeries` in vol.js) adds `volatility: {annualPct, monthPct}` to `/api/history`; the detail panel shows "Expected swing (EWMA vol) ±X% · typical month"; the legend explains it forecasts bumpiness, never direction.
- **Phase 5b closed (v0.12.0): up/down probabilities measured — the base rate won.** `server/harness/prob.js` tested the Christoffersen–Diebold drift-over-vol model (P(up)=Φ(μh/σ√h), expanding drift, EWMA vol, nothing fitted) against expanding-window climatology on Brier/log-loss at 1w/1m/3m/1y, ship rule pre-committed in the script header: the vol layer scored WORSE at every horizon (beat climatology 1/18, 0/18, 2/18, 8/18; same total-return) — high-vol moments are rebound moments, so vol-shrinking P(up) toward 50% fires backwards; the Gaussian ablation also lost to plain counting (skew). Climatology beat the coin 15/14/13 of 18 at 1w/1m/3m. Shipped: the report card shows per-ticker "odds of being higher" at 4 horizons (base rates via `export.js`, which now also prints in the constant's exact paste shape), labeled "not a forecast of this moment"; legend explains the negative result. `vol.js` now exports its helpers (CLI guarded by `require.main`) for prob.js. Digest: `docs/research/calibration.md`. Never show a "P(up)" that isn't the base rate unless a model beats climatology out-of-sample first.
- **Phase 5c closed (v0.13.0): the weighted multi-signal model was measured and declined.** `server/harness/combo.js` fitted the one pre-committed spec — ridge logistic regression from 4 standardized features (MA state ±1/0, (RSI−50)/50, MACD cross ±1, ln EWMA vol) to P(up in 1w/1m/3m) — strictly inside `walkforward.js` training windows (5y/1y sliding, deterministic IRLS, intercept learns the base rate so features must add info beyond climatology). Verdict: **0/18 tickers beat climatology on Brier at any horizon** (same on log-loss and total-return); damage grows with horizon (median BSS −1.2%/−4.2%/−10.2%) and shrinks as ridge tightens — the best model is the one shrunk back to climatology; no feature exceeds 65% sign agreement across 270 fits. The fitted-vol-rebound follow-up from `calibration.md` §5 is answered: leans positive at 3m, survives nothing. No UI score ships (per ship rule + the standing base-rate rule); the legend expander records the null in one sentence. Digest: `docs/research/multi-signal.md` (8 citations). Congressional flow deliberately excluded from the features (known-null aggregate, ~1y of per-ticker data; 5e measures per-member records instead).
- **Phase 5d closed (v0.14.0): volatility-targeted sizing — the second measured-positive result, and the first strategy-shaped one.** `server/harness/voltarget.js` (ship rule pre-committed): w = min(1, expanding-median EWMA vol / current EWMA vol), long-only, no leverage, 5pp rebalance band, 0.1% cost on traded volume, nothing fitted (no walk-forward needed). Verdict: Sharpe beats the matched constant-exposure control (same average exposure — isolates timing) 12/18 plain / 16/18 total-return; max drawdown shallower than B&H 18/18 on both price sets (median −37% vs −59%); crisis wins 18/17/15 of 18; robust to --power/--band/--lambda/--cost ablations; honest cost ~2.4pp/yr raw CAGR at ~85% avg exposure; laggards are the rebound-heavy tickers (XOM/INTC/PG/T). `metrics.js` gained `sortino()` + `annualVol()`; `backtest.js` gained `simulateWeights()` (fractional exposure, costs on traded volume — binary weights reproduce `simulate()` exactly). Shipped: `export.js` adds per-ticker `vt` (target = median vol as ±%/month + measured Sharpe/DD pair) to HARNESS_RESULTS; the report card's "sizing by calm vs. storm" line shows the frozen numbers plus live implied exposure (target ÷ current monthPct); legend explains the trade-off. Labeled risk illustration, never advice, never direction. Digest: `docs/research/risk-sizing.md` (10 citations). Phase 6's forward test is the pre-registered final arbiter.
- **Phase 5e closed (v0.15.0): per-member congressional track records — the post-2012 null, reproduced.** `server/harness/congressdata.js` (multi-year corpus: all 5,849 digital House PTRs 2014–2026 → 43,430 deduped trades, 256 members after name canonicalization — dual-format PDF parser, keep its regex in sync with server.js parseTransactions; `memberKey()` is mirrored in index.html) + `server/harness/congresstrack.js` (scoring per `congressional-trading.md` §6, method pre-committed: disclosure-date clock, excess vs SPY at 1m/3m/6m/12m, equal weight, ≥20-buy gate, matched-random percentiles, best-of-N, split-sample; `--export` prints index.html's CONGRESS_RECORDS constant). Verdict in §7: at 34% coverage, pooled buys +1.20% at 6m on a 50% hit rate. **Coverage extension completed 2026-07-25 (v0.22.0) and the deflator turned out to be most of the signal — see §7.1:** 666 cached tickers → **69% coverage** (29,804/43,430 scored), pooled buys fall to **+0.15% at 6m (n=14,466, 47% hit)**, essentially reproducing Belmont et al.'s −0.26% null; 6/96 members ≥95th luck pctile vs ~5 by chance; best-of-N now decisive — the top record (+14.28%) sits at the **24th pctile** of best-of-96-random (median 17.31%), i.e. weaker than luck typically manufactures; split-sample top decile decayed out-of-sample. **No skill claims; the UI ships records-with-luck-percentiles as transparency and now prints the coverage % in the pooled footer, because coverage is load-bearing (34%→69% cut the headline ~90%).** Always quote coverage alongside any statistic from this corpus. Residual gaps are disproportionately delisted names, so +0.15% is an upper bound. Coverage extendable further via `congresstrack.js --maxfetch=N` on a fresh Twelve Data day (negative cache in cache/congress/missing-tickers.json, now 89 dead tickers).
- **Browse-by-category tier (v0.16.0, first Phase-7 item, owner-requested):** below the watchlist, compact quote-only rows grouped by category — the 18 measured names in 4 static groups plus "Congress favorites" from the new `/api/congress/popular` endpoint (top 12 tickers by House trade count, 365d window; registered before `/:ticker` like `/recent`). Two-tier data model: rows fetch only cached quotes (4-at-a-time), full data loads on row-click via `openBrowseTicker()` (STOCKS entry without watchlist membership — `renderCard()` now guards `watchlist.includes()` so no phantom cards). `addTickerByName()` is the shared add path.
- **Phase 6a shipped (v0.17.0): the manual paper portfolio.** Frontend-only panel (`paper_v1` in localStorage): $10k fake cash, buy-by-dollars/sell-all at live quotes, 0.1% fee per trade (harness cost model), valuation on load vs a price-only SPY-since-inception benchmark (dividend caveat in the footer). **Every trade stores a `snap` of what the dashboard said at execution** (signal label, monthPct swing, vol-sized exposure %) — these snapshots are the grading records for the Phase-5 forward test; never delete them lightly. Key functions: `startPaper/paperBuy/paperSell/renderPaper/refreshPaperQuotes`; buys await `refreshSignal()` so snapshots aren't empty.
- **Phase 6b shipped (v0.18.0, made ticker-selectable in v0.20.0): the auto-follow account** (`auto_v1` in localStorage): mechanically holds ONE user-chosen ticker (`auto.ticker`, default/legacy SPY — `autoTicker()`) at min(1, target/forecast). The picker offers only the 18 measured names (each needs a frozen `vt.target`) and labels the two where the rule measured worse than holding (XOM, INTC) rather than hiding them; panel copy is deliberately plain-language after owner feedback that the original was unreadable, and the running stance distinguishes what the rule CALLS FOR from what the account HOLDS (they differ within the 5pp band) — parameters mirror voltarget.js (keep in sync: 5pp band, 0.1% cost, target from HARNESS_RESULTS.tickers.SPY.vt.target, forecast = live monthPct from /api/history/SPY). Evaluated in `autoCheck()` on page load, at most once per calendar day (`lastCheck` guard; buy-side delta capped by cash/(1+cost) so fees can't overdraw). Rebalance log + always-100%-SPY benchmark; can only de-risk. Phase 6 is feature-complete for its first iteration.
- **Snapshot-grading report shipped (v0.19.0, 2026-07-25):** a panel that grades the dashboard's own shipped claims forward against real closes at 1w/1m/3m/6m — the volatility band (`volatility.md` §5.1), the signal label (expected null, `ma-timing.md` §7), and the vol-sizing account (`risk-sizing.md` §6.1). **Grading rules were pre-committed on 2026-07-25 while the accounts were 7 days old and nothing had matured** (method in the `renderGrade` header block + `docs/research/README.md`); pre-committed minimum samples (20 band / 30 per signal bucket / 126 trading days for risk) below which it shows numbers but refuses a verdict. New `GET /api/closes/:ticker?from=YYYY-MM-DD` supplies dated closes; the 6b log now stores post-trade `cash`/`shares` so the account's daily value path replays exactly. Returns are measured close-to-close from ONE price source, not the recorded execution price (the quote feed's ~2% offset would exceed the move at 1w). Grading fetches share Twelve Data's 8-req/min budget with the harness scripts — don't run a browser verify against a local backend while `congresstrack.js` is fetching.
- **Band calibration measured (v0.21.0, 2026-07-25, `volband.js`):** the UI's "about 2 months in 3 land inside the expected-swing band" was a calibration claim that shipped untested — `vol.js` had only measured *ranking*. Verdict (`volatility.md` §6, method pre-committed): **68.8% of 4,068 non-overlapping ticker-months landed inside +/-1sigma vs 68.3% normal theory**; 17/18 tickers inside the pre-committed 63-73% range; robust to --adjust (68.4%), --horizon=5 (70.3%), --lambda=0.97 (70.6%). Nothing shipped — first pre-committed check to return "no action needed". Corrected `volatility.md` §5.1: fat tails are real but live at +/-2sigma (94.0% vs 95.4%), invisible at the 1sigma band displayed. Use NON-OVERLAPPING samples for any coverage stat — overlapping 21d windows share 20/21 outcome days and overstate precision ~21x.
- **Next planned tasks:** wait for horizons to mature and read the grading report (grade 6a trade `snap`s and the 6b account once positions age past 1m/3m/6m horizons — needs a small backend addition to expose closes-since-a-date, or grading against /api/history sparkline windows), extending 5e price coverage (in progress), then the rest of Phase 7 UI polish.
...
| `server/server.js` | The backend. Endpoints: `GET /api/quote/:ticker` (live), `GET /api/history/:ticker` (history + signal + RSI/MACD), `GET /api/news/:ticker` (headlines), `GET /api/congress/recent` (cross-ticker feed), `GET /api/congress/popular` (most-traded tickers, browse category), `GET /api/congress/:ticker` (House trades), `GET /api/closes/:ticker?from=` (dated closes for the Phase-6 grading report), `GET /health`. |
...
node vol.js                   # EWMA volatility forecastability test (--adjust,
                              # --lambda=0.94, --horizon=21)
node volband.js               # is the DISPLAYED +/-1sigma band the right size?
                              # non-overlapping coverage vs 68.3% (--adjust,
                              # --lambda=, --horizon=)
node prob.js                  # up/down probability calibration vs climatology
                              # (--adjust, --lambda=0.94)
node combo.js                 # 5c multi-signal walk-forward ridge logit vs
                              # climatology (--adjust, --ridge=0.01, --train=5
                              # --test=1, --anchored)
node voltarget.js             # 5d vol-targeted sizing vs B&H + matched
                              # constant-exposure control (--adjust, --power=1,
                              # --band=0.05, --lambda=0.94, --cost=0.001)
node congressdata.js          # fetch/refresh the 2014-2026 House PTR corpus
node congresstrack.js         # 5e per-member track records (--maxfetch=600,
                              # --minbuys=20, --trials=1000, --export)
node export.js                # regenerate the UI report-card constant (paste into
                              # index.html's HARNESS_RESULTS)
```
