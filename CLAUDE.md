...
- **Phase 5e closed (v0.15.0): per-member congressional track records — the post-2012 null, reproduced.** `server/harness/congressdata.js` (multi-year corpus: all 5,849 digital House PTRs 2014–2026 → 43,430 deduped trades, 256 members after name canonicalization — dual-format PDF parser, keep its regex in sync with server.js parseTransactions; `memberKey()` is mirrored in index.html) + `server/harness/congresstrack.js` (scoring per `congressional-trading.md` §6, method pre-committed: disclosure-date clock, excess vs SPY at 1m/3m/6m/12m, equal weight, ≥20-buy gate, matched-random percentiles, best-of-N, split-sample; `--export` prints index.html's CONGRESS_RECORDS constant). Verdict in §7: pooled buys +1.20% at 6m on a 50% hit rate (34% trade coverage = most-traded ~200 tickers, survivor-tilted, stated in UI); 7/77 members ≥95th luck percentile vs ~4 by chance; best-of-N fails (top record = 93rd pctile of best-of-77-random); split-sample suggestive only. **No skill claims; the UI ships records-with-luck-percentiles as transparency.** Price coverage can be extended incrementally (rerun `congresstrack.js --maxfetch=N` on a fresh Twelve Data day; negative cache in cache/congress/missing-tickers.json).
