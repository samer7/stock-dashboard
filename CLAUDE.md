...
- **Next planned tasks:** the snapshot-grading report (grade 6a trade `snap`s and the 6b account once positions age past 1m/3m/6m horizons — needs a small backend addition to expose closes-since-a-date, or grading against /api/history sparkline windows), extending 5e price coverage (in progress), then the rest of Phase 7 UI polish.
...
| `server/server.js` | The backend. Endpoints: `GET /api/quote/:ticker` (live), `GET /api/history/:ticker` (history + signal + RSI/MACD), `GET /api/news/:ticker` (headlines), `GET /api/congress/recent` (cross-ticker feed), `GET /api/congress/popular` (most-traded tickers, browse category), `GET /api/congress/:ticker` (House trades), `GET /health`. |
