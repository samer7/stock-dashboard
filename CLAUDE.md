...
- Two of the digest's falsifiable follow-ups are tested (v0.9.5): the **crisis-window test** confirms the drawdown claim decisively (daily rule beats buy-and-hold in 49/54 crisis ticker-windows, e.g. median −9.5% vs −50.5% in the financial crisis), and **Faber's 10-month SMA** (`--strategy=faber`) behaves exactly as the literature predicts (loses on return 0/6 total-return, keeps the drawdown benefit with 4–8× fewer trades). The MA topic's digest→implement→verify loop is closed.
- **Next planned tasks:** 12-month TSMOM (last untested digest follow-up), then the momentum and congressional research digests, walk-forward machinery, 5b–5e.
